...
*Fuente: [antvis/chart-visualization-skills](https://github.com/antvis/chart-visualization-skills). Para referencias detalladas por tipo de gráfico y el script `generate.js`, clonar el repo o ver la carpeta `skills/chart-visualization/` en GitHub.*
...
Analyze the user's data features to determine the most appropriate chart type. Use the following guidelines (and consult `references/` in the repo for detailed specs):
...
Once a chart type is selected, read the corresponding file in the `references/` directory in the repo (e.g., `references/generate_line_chart.md`) to identify the required and optional fields.
Extract the data from the user's input and map it to the expected `args` format.

### 3. Chart Generation
Invoke the `scripts/generate.js` script with a JSON payload (script is in the repo at `skills/chart-visualization/scripts/generate.js`).
...
**Execution Command:**
```bash
node ./scripts/generate.js '<json_payload>'
```
...
## Reference Material
Detailed specifications for each chart type are in the repo under `skills/chart-visualization/references/`. Consult those files to ensure the `args` passed to the script match the expected schema.
