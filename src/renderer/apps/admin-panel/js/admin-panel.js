...
class AdminPanel {
  ...
  getAppDisplayName(appName) {
    const displayNames = {
      dashboard: "Dashboard",
      "Inventory-Healt": "Inventory Health",
  ...
  populateTraditionalPermissions(username = null) {
    ...
    // Header sin botón de cambio de vista
    const headerHtml = '<h3 style="margin-bottom: 1rem; color: var(--text-color-three);">Permisos de Aplicación</h3>';
  ...
  if (window.permisosService && !window.permisosService.esAdmin()) {
    ...
    appContainer.innerHTML = `
      <div style="display: flex; align-items: center; justify-content: center; height: 100vh; text-align: center; padding: 2rem;">
        <div style="background: var(--sidebar_bg_ch, #f8f9fa); padding: 2rem; border-radius: 0.5rem; border: 0.0625rem solid var(--Limit-Sidebar, #ddd);">
